...
function applyFormatOptions(htmlContent, formatOptions) {
  ...
  if (formatOptions.removeExtraBreaks) {
    ...
    for (let i = brElements.length - 1; i >= 0; i--) {
      ...
      if (nextElement && nextElement.tagName === "BR" || nextSibling && nextSibling.nodeType === Node.ELEMENT_NODE && nextSibling.tagName === "BR") {
        br.remove();
      }
    }
    ...
    for (let i = 0; i < emptyParagraphs.length; i++) {
      ...
      const prevIsEmpty = prev && (prev.tagName === "P" || prev.tagName === "DIV") && prev.textContent.trim() === "" && prev.children.length === 0;
      const nextIsEmpty = next && (next.tagName === "P" || next.tagName === "DIV") && next.textContent.trim() === "" && next.children.length === 0;
...
\u8D85\u65F6\u65F6\u95F4: ${this.settings.timeout}ms`;
  ...
  async previewCurrentDocument(coverImage = "") {
    ...
    try {
      ...
      const formatOptions = (sidebarView == null ? void 0 : sidebarView.formatOptions) || { enabled: false };
      if (formatOptions.enabled) {
        console.log("\u5E94\u7528\u6392\u7248\u9009\u9879...");
